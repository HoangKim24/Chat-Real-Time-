...
const ChannelSidebar = ({ onChannelClick }: ChannelSidebarProps) => {
  ...
  const handleChannelClick = (conversation: typeof conversations[0]) => {
          ...
          {channels.length > 0 && (
            <div>
              <div className="flex items-center text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 px-2 hover:text-slate-300 transition-colors cursor-pointer group">
                ...
                <button
                  onClick={(event) => { event.stopPropagation(); setAddChannelModalOpen(true); }}
          ...
          <div>
            ...
            <div className="space-y-0.5">
              {conversations.map((conversation) => (
                <div
                  key={conversation.id}
                  onClick={() => handleChannelClick(conversation)}
                ...
                >
                  ...
                  <span className="text-sm font-medium truncate">{conversation.name || `Hội thoại #${conversation.id}`}</span>
                </div>
              ))}
