...
const ChatArea = () => {
  ...
  const { messages: apiMessages, fetchMessages, sendMessage, editMessage, deleteMessage, clearMessages, isLoading } = useMessageStore();
  ...
  useEffect(() => {
    ...
    clearMessages();
  }, [activeConversation?.id, clearMessages, fetchMessages]);
  ...
  const localMessages = useMemo<LocalMessage[]>(() => (
    apiMessages.map((message) => ({
      ...
      senderAvatar: message.senderAvatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(message.senderName || 'Người dùng')}&background=1f2937&color=fff`,
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-transparent animate-fade-in">
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-4 flex flex-col gap-1 relative scrollbar-thin">
        ...
        {activeConversation && !isLoading && localMessages.length === 0 && (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center max-w-md">
              <div className="w-20 h-20 mx-auto mb-4 bg-white/5 rounded-3xl flex items-center justify-center">
                <svg fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" width="40" height="40" className="text-slate-500"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg>
              </div>
              <h3 className="text-lg font-bold text-white/70 mb-1">Chưa có tin nhắn nào</h3>
              <p className="text-sm text-slate-500">Hãy gửi tin nhắn đầu tiên để bắt đầu cuộc trò chuyện trong {channelName}.</p>
            </div>
          </div>
        )}

        {activeConversation && localMessages.length > 0 && (
          <div className="flex flex-col gap-0.5 mt-auto">
            {localMessages.map((message) => (
              <div key={message.id} className={`flex items-start gap-3 group animate-slide-up py-1.5 px-2 hover:bg-white/[0.02] rounded-lg transition-colors ${message.isMe ? 'flex-row-reverse' : ''}`}>
                {!message.isMe && (
                  <div className="relative shrink-0 mt-1">
                    <img src={message.senderAvatar} alt="avatar" className="w-9 h-9 rounded-xl shadow-lg border border-white/10" />
                ...
                <div className={`flex flex-col max-w-[70%] ${message.isMe ? 'items-end' : 'items-start'}`}>
                  {!message.isMe && (
                    <div className="flex items-baseline space-x-2 mb-1.5 px-1">
                      <span className="text-sm font-semibold text-white group-hover:text-cyan-400 transition-colors cursor-pointer">{message.senderName}</span>
                      <span className="text-[11px] text-slate-500 font-medium" title={new Date(message.createdAt).toLocaleString()}>{formatTime(message.createdAt)}</span>
                      {message.isEdited && <span className="text-[10px] text-slate-600 italic">(sửa)</span>}
                    </div>
                  )}
                  ...
                  <div className={`relative group/msg flex items-center ${message.isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                    <div className={`flex flex-col ${message.isMe ? 'items-end' : 'items-start'}`}>
                      {message.replyTo && (
                        <div className={`mb-1 px-3 py-1 bg-white/5 border-l-2 border-cyan-500 rounded-lg text-[13px] text-slate-400 italic max-w-full truncate flex items-center gap-2 opacity-80 ${message.isMe ? 'mr-1' : 'ml-1'}`}>
                          <span className="font-bold text-cyan-500/80 not-italic text-[11px] uppercase tracking-tighter">Trả lời {message.replyTo.user}:</span>
                      ...
                      {editingMessage?.id === message.id ? (
                        ...
                        <div className={`px-3.5 py-2 rounded-2xl text-sm leading-relaxed shadow-sm transition-all duration-300 ${
                          message.isMe ? 'bg-cyan-600 text-white rounded-br-sm' : 'bg-slate-800/80 text-slate-200 rounded-bl-sm hover:bg-slate-800'
                        }`}>
                          {message.content}
                        </div>
                      )}
                    ...
                    <div className={`absolute top-0 flex items-center bg-slate-900/90 backdrop-blur-2xl rounded-2xl shadow-2xl border border-white/10 opacity-0 group-hover/msg:opacity-100 transition-all duration-300 p-1 pointer-events-none group-hover/msg:pointer-events-auto z-20 ${message.isMe ? 'right-0 -translate-y-[110%]' : 'left-0 -translate-y-[110%]'}`}>
                      <div className="relative">
                        <button
                          onClick={() => setShowEmojiPicker(showEmojiPicker === message.id ? null : message.id)}
                          className="p-2 text-slate-400 hover:text-cyan-400 hover:bg-white/10 rounded-xl transition-all active:scale-90"
                          title="Thêm phản ứng"
                        >
                      ...
                      {message.isMe && (
                        <>
                          <button
                            ...
                            className="p-2 text-slate-400 hover:text-yellow-400 hover:bg-white/10 rounded-xl transition-all active:scale-90"
                            title="Chỉnh sửa"
                          >
                          ...
                          <button
                            ...
                            className="p-2 text-slate-400 hover:text-red-400 hover:bg-white/10 rounded-xl transition-all active:scale-90"
                            title="Xóa"
                          >
                  ...
                  {message.reactions.length > 0 && (
                    <div className={`flex flex-wrap gap-1.5 mt-1 px-1 ${message.isMe ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <div className="relative">
                  <button
                    ...
                    className="text-slate-400 hover:text-cyan-400 p-2 hover:bg-white/5 rounded-xl transition-all active:scale-90"
